    ...
    rellaFullpage.prototype = {
        ...
        build: function() {
            if (!/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
                ...
                var navTooltips = [], sections = el.find(options.sectionSelector), mainHeader = $('.main-header'), mainFooter = $('.main-footer'), pageNavs = $('.main-header, body'), backToTop = $('.site-backtotop'), mainNav = $('.main-nav', mainHeader), localScrollLinks = $('.local-scroll', mainNav);
                ...
                el.fullpage($.extend(true, {}, this.options, {
                    ...
                    onLeave: function(index, nextIndex, direction) {
                        ...
                        if (nextIndex > 1 && backToTop.length) {
                ...
                if (localScrollLinks.length) {
                    localScrollLinks.on('click', 'a', function(event) {
                        event.preventDefault();
                        var $this = $(this), href = $this.attr('href'), targetSection = $(href, sections), index = $('.section', '#content').index(targetSection) + 1;
...
(function($) {
    ...
    AnimatedIcon.prototype = {
        ...
        build: function() {
            var self = this, el = this.el, obj = el.find('svg'), file = self.options.file, objVivus, delayTime = parseInt(self.options.delay, 10), canAnimate = self.options.animated, isCustomColorApplied = self.options.customColorApplied, parentIconbox = el.closest('.icon-box').attr('id', 'iconbox-' + Math.round(Math.random() * 1e6)), element = file ? this.el.get(0) : obj.get(0);
            if (!file && !obj.length) return;
            objVivus = new Vivus(element, {
                type: self.options.type,
                file: file,
                duration: self.options.duration,
                start: 'manual',
                onReady: function(vivus) {
                    var strokegradients, strokeHoverGradients = document.createElementNS('http://www.w3.org/2000/svg', 'style'), linearGradientEl = document.createElementNS('http://www.w3.org/2000/svg', 'linearGradient'), gradientValues = typeof self.options.color !== typeof undefined && self.options.color !== null ? self.options.color.split(',') : '#000', hoverGradientValues = self.options.hoverColor, gid = Math.round(Math.random() * 1e6), $svg = $(vivus.el);
                    if (undefined === gradientValues[1]) {
                        gradientValues[1] = gradientValues[0];
                    }
                    strokegradients = '<defs xmlns="http://www.w3.org/2000/svg"><linearGradient gradientUnits="userSpaceOnUse" id="grad' + gid + '" x1="0%" y1="0%" x2="0%" y2="100%">' + '<stop offset="0%" stop-color="' + gradientValues[0] + '" />' + '<stop offset="100%" stop-color="' + gradientValues[1] + '" />' + "</linearGradient></defs>";
                    var xmlStrokegradients = new DOMParser().parseFromString(strokegradients, "text/xml");
                    $svg.prepend(xmlStrokegradients.documentElement);
                    if (typeof undefined !== typeof hoverGradientValues && null !== hoverGradientValues) {
                        ...
                        $svg.prepend(strokeHoverGradients);
                    }
                    if (isCustomColorApplied) {
                        $svg.find('path, rect, ellipse, circle, polygon, polyline').attr({
                            stroke: 'url(#grad' + gid + ')',
                            fill: 'none'
                        });
                    }
                    $svg.closest('.icon-container').addClass('appear-animation-visible');
                    $svg.css({
                        opacity: 1
                    });
                }
            }).setFrameProgress(1);
            if ($(window).width() >= 992 && canAnimate) {
                objVivus.reset().stop();
                var inViewCallback = function(enteries, observer) {
                    enteries.forEach(function(entery) {
                        if (entery.isIntersecting) {
                ...
                new IntersectionObserver(inViewCallback, {
                    threshold: .25
                }).observe(el.get(0));
...
(function($) {
    ...
    CircleProgressbar.prototype = {
        ...
        progressbarCircle: function() {
            var $element = this.el, inner = $element.find('.progressbar-inner'), percentage = $element.attr('data-percentage') + '%', circleSize = $element.parent().width() > 0 ? $element.parent().width() - 20 : 150;
            var progress = inner.circleProgress({
                value: 0,
                size: circleSize,
                thickness: $element.data('thickness'),
                startAngle: Math.PI * 3.501411705537642,
                emptyFill: $element.data('empty-fill'),
                animation: {
                    duration: 1100
                },
                reverse: $element.data('reverse') === true ? true : false,
                fill: {
                    gradient: [ $element.data('start-color'), $element.data('end-color') ],
                    gradientAngle: Math.PI * 3.501411705537642
                }
            });
            var inViewCallback = function(enteries, observer) {
                enteries.forEach(function(entery) {
                    if (entery.isIntersecting && !$element.hasClass('animate-started')) {
                        inner.circleProgress({
                            value: parseInt(percentage, 10) / 100
                        });
                        $element.addClass('animate-started');
                    }
                });
            };
            var options = {
            ...
            observer.observe(observerTarget);
            $(window).on('resize', function() {
                $element.find('.progressbar-inner').circleProgress('redraw');
            });
            $(document).on('shown.bs.tab', 'a[data-toggle="tab"]', function() {
                $element.find('.progressbar-inner').circleProgress({
    ...
    $(document).ready(function() {
        $('.progressbar-circle').rellaCircleProgressbar();
    });
}).apply(this, [ jQuery ]);
        ...
        build: function() {
            ...
            var progressive = progressively.init({
                ...
                onLoad: function(el) {
                    ...
                    setTimeout(function() {
                        if ($(el).closest('[data-plugin-masonry]').not('.timeline-row').length && $(el).closest('[data-plugin-masonry]').data('isotope')) {
...
(function($) {
    ...
    Header.prototype = {
                ...
                if (typeof dataTarget === typeof undefined || dataTarget === null) {
                    ...
                    moduleTrigger.on('click', function(event) {
                        ...
                        if (!$this.closest('.style-ghost').length) {
                            ...
                            var siblingModules = [], searchSiblings = [];
                            if (window.elementorFrontend) {
                                var $parentElementorElement = $this.closest('.elementor-element');
                                var $parentElementorSection = $this.closest('.elementor-section');
                                var $siblingLogoModule = $parentElementorSection.siblings('.elementor-widget-boo_header_logo');
                                siblingModules = $parentElementorElement.prevAll('.elementor-element');
                                if ($parentElementorElement.siblings('.elementor-widget-boo_main_nav').find('.main-nav').children('li').length) {
                                    searchSiblings.push($parentElementorElement.siblings('.elementor-widget-boo_main_nav').find('.main-nav').children('li'));
                                }
                                if ($siblingLogoModule.length) {
                                    var logoWidth = $siblingLogoModule.outerWidth();
                                    if (!isRTL) {
                                ...
                                var $modulesContainer = $this.closest('.modules-container');
                                var $siblingLogoModule = $modulesContainer.siblings('.navbar-brand');
                                siblingModules = $this.prevAll('div');
                                if ($modulesContainer.siblings('.navbar-collapse').find('.main-nav').children('li').length) {
                                    searchSiblings.push($modulesContainer.siblings('.navbar-collapse').find('.main-nav').children('li'));
                                }
                                if ($siblingLogoModule.length) {
                                    var logoWidth = $siblingLogoModule.outerWidth();
                                    if (!isRTL) {
                            ...
                            $(siblingModules.not('.elementor-widget-boo_main_nav').get().reverse()).each(function() {
                                var $this = $(this);
        ...
        desktopHeaderSubmenu: function() {
            var mainHeader = $('.main-header'), mainBarContainer = mainHeader.find('.main-bar-container'), mainNav = mainHeader.find('.main-nav, .custom-menu'), submenu = mainNav.find('.nav-item-children, .children'), megamenuParent = mainNav.find('.megamenu'), isRTL = $('html').attr('dir') == 'rtl';
            mainBarContainer.each(function() {
                var $this = $(this), backLabel = $this.attr('data-back-label') || 'Back';
            ...
            submenu.add(mainHeader.find('.custom-menu').find('.sub-menu')).each(function() {
                ...
                if (!mainBarContainer.is('.main-bar-side')) {
                    ...
                    var arrow = $('<i class="nav-toggle"></i>');
                    subParent.children('a').append(arrow);
                    subParent.children('a').on('click', '.nav-toggle', function(e) {
                        e.preventDefault();
                        e.stopPropagation();
                        var mainHeader = $('.main-header');
                        if (mainHeader.hasClass("desktop")) {
                            var $liParent = $(this).closest('li'), navChild = $liParent.children('.nav-item-children');
                            if (navChild.length) {
                                $liParent.toggleClass('submenu-is-showing').siblings().removeClass('submenu-is-showing').find('.nav-item-children').hide();
                                navChild.stop().slideToggle(300);
                            }
                        }
                    });
                }
        ...
        mobileSubmenuAddToggleBtn: function() {
            ...
            $('.nav-item-children, .children', mobileMainHeader).each(function(i, element) {
                ...
                el.each(function() {
                    ...
                    $this.imagesLoaded(function() {
                        ...
                        hr = new Headroom($this.get(0), {
                            ...
                            offset: $this.attr('data-start-point') == 'start' ? $this.offset().top : $(".main-header").outerHeight() + $this.offset().top,
                            onPin: function() {
                                stickyPlaceholder.attr({
                                    'data-height': '',
                                    'data-sticky-offset-top': 0
...
(function($) {
    ...
    EnableMediaElementJs.prototype = {
        ...
        build: function() {
            ...
            if (element.parents('.rev_slider_wrapper').length) {
                return;
            }
...
(function($) {
    ...
    LocalScroll.prototype = {
        ...
        build: function() {
            var el = $('.local-scroll'), mainHeader = $('.main-header'), mainbarContainer = $('.main-bar-container', mainHeader), mainbarIsSticky = mainbarContainer.length && mainbarContainer.is('[data-sticky]'), isOnePageNav = $('.main-nav').find('.local-scroll').length || $('.custom-menu').find('a:not([href="#"]):not([href="#0"])').length ? true : false;
            if (isOnePageNav) {
                $('body').scrollspy({
            ...
            el.on('click', 'a:not([href="#"]):not([href="#0"])', function(event) {
                ...
                if (location.pathname.replace(/^\//, '') == this.pathname.replace(/^\//, '') && location.hostname == this.hostname) {
                    ...
                    if (mainbarIsSticky) {
                        offset = offset - mainbarContainer.outerHeight();
                    }
                    if (target.length) {
                        event.preventDefault();
                        $('html, body').animate({
...
(function($) {
    var Rella = {
        ...
        rellaAnimations: function() {
            ...
            if (rellaAnimateElement.length) {
                ...
                jQuery.fn.vcwaypoint && rellaAnimateElement.vcwaypoint(function() {
                    var $element = $(this.element);
        ...
        preloader: function() {
            ...
            var removePreloader = function() {
                ...
                if (preloaderStyle1.length) {
                    var onPageLoad = Rella.debounce(function() {
                        $(window).triggerHandler('resize');
        ...
        accordion: function() {
            var isInsideTabs = false;
            if (location.hash !== '' && $(location.hash).length) {
                var $element = $(location.hash);
                var $tabPaneParent = $element.closest('.tab-pane');
                if ($tabPaneParent.length) {
                    isInsideTabs = true;
                    var tabPaneID = $tabPaneParent.attr('id');
                    var $tabsNav = $tabPaneParent.closest('.tabs').find('.nav');
                    var $tabLink = $('a[aria-controls=' + tabPaneID + ']', $tabsNav);
                    $tabLink.tab('show');
                ...
                var activeItemParent = $element.closest('.accordion-item').first();
            ...
            $('.accordion-collapse').on('show.bs.collapse', function(e) {
                var $this = $(this);
                $this.closest('.accordion-item').first().addClass('active');
            ...
            $('.accordion-collapse').on('shown.bs.collapse', function(e) {
                var $this = $(this), parent = $this.closest('.accordion-item').first(), $window = $(window), parentOffsetTop = parent.offset().top;
                if (parentOffsetTop <= $window.scrollTop() - 15 && !isInsideTabs) {
                    $('html, body').animate({
            ...
            $('.accordion-collapse').on('hide.bs.collapse', function() {
                $(this).closest('.accordion-item').first().removeClass('active');
            });
        },
        tabs: function() {
            if (location.hash !== '' && $(location.hash).length) {
            ...
            $('[data-toggle=tab]').on('shown.bs.tab', function(e) {
                ...
                var $pie_charts = target.find(".vc_pie_chart:not(.vc_ready)"), $round_charts = target.find(".vc_round-chart"), $line_charts = target.find(".vc_line-chart"), $gallery_slide = target.find('.wpb_gallery_slides'), $flex_slider = target.find('.wpb_flexslider');
                ...
                if ($flex_slider.length) vc_plugin_flexslider();
            });
        },
        postShare: function() {
    ...
    var wooButtonUpdate = function() {
        'use strict';
        var button = $('.woocommerce-cart .woocommerce .update_cart');
        if (!button.length) {
            return;
        }
        if (button.attr('disabled') == 'disabled') {
            button.prop('disabled', false);
            var but = button.parent().find('.input-generated-button');
            if (but.length) {
    ...
    $(window).on('resize', function() {
        Rella.themeRTL();
        Rella.blogCoverSidebar();
    });
    $(window).on('load', function() {
        Rella.objectFitPolyfill();
    });
})(jQuery);