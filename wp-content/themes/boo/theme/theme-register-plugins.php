...
if( 'valid' != get_option( get_template() . '_license_key_status', false ) ) {
	return;
}
...
function _s_register_required_plugins() {
	...
	$plugins = array(
		...
		array(
			...
			'required' 	        => true,
			'source'            => get_template_directory_uri() . '/plugins/infinite-addons.zip',
		...
		array(
			...
			'version'           => '1.2',
			'required' 	        => false,
			'source'            => get_template_directory_uri() . '/plugins/infinite-portfolio.zip',
			'rella_logo'        => $images . '/extension-placeholder.jpg',
			'rella_author'      => 'Themerella',
