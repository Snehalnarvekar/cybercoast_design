...
class RellaDownload
{
	 ...
	 * @var string
	 */
	private $temp_folder_name = 'rella_temp';
	...
	{
		$this->core = $core;
		$this->api_url = 'http://apitwo.themerella.com/';
	}

	public function run() {
		add_action( 'wp_ajax_rella_prepare_demo_package', array($this, 'ajax_prepare_demo'), 10, 1 );
	}
	...
	public function download($demo){
		...
		$args = array(
			'timeout'    => 30,
			'user-agent' => 'themerella-user-agent',
		);
	...
	public function ajax_prepare_demo() {
		...
		$is_valid = get_option( get_template() . '_license_key_status', false );
		$ret = '';
		

		if( 'valid' != $is_valid ) {
			$ret = '{"stat":0, "message":"Please activate your Boo License"}';
			echo $ret;
			wp_die();
		}
		...
		if( !$download ) {
			$ret = '{"stat":0, "message":"Your server was unable to connect to ThemeRella. Please check with your hosting company if the connection to https://api.themerella.com is blocked in the firewall or network setup."}';
			echo $ret;
			wp_die();
		}
		...
		if( isset($download['content-length']) && $download['content-length'] >= 1 ) {
			...
			$ret = '{"stat":0, "message":"Failed to download the demo package or the downloaded file is corrupted, Contact ThemeRella support !"}';
		}
		echo $ret;
		wp_die();
	}
