 ...
 */
function rella_get_recent_tweets($username, $consumer_key, $consumer_secret, $access_token, $access_token_secret, $cache_time = 3600) {
	...
	{
		require_once rella_addons()->plugin_dir() .'/extras/class/tmhOAuth.php';
		require_once rella_addons()->plugin_dir() .'/extras/class/tmhUtilities.php';
 ...
 * @return string
 */
function rella_get_linkedin_connections( $api_key, $api_secret ) {
 ...
 * @return null
 */
function rella_replace_in_tweets($tweet) {
	...
	if (!empty($tweet->entities->urls) || !empty($tweet->entities->hashtags) || !empty($tweet->entities->user_mentions)) {
		...
		$tweet_text = rella_replace_urls_with_html($tweet_text);
 ...
 * @return string
 */
function rella_replace_urls_with_html($text) {
	...
	return preg_replace_callback("&\\b$rexProtocol$rexDomain$rexPort$rexPath$rexQuery$rexFragment(?=[?.!,;:\"]?(\s|$))&", 'rella_replace_urls_with_html_callback', htmlspecialchars($text));
}
 ...
 * @return type
 */
function rella_replace_urls_with_html_callback($match) {
 ...
 * @return type
 */
function rella_time_elapsed_string($datetime, $full = false) {
	$now = new DateTime;
 ...
 * @return string
 */
function rella_get_remote_html( $url = '' ) {

	// Check for transient, if none, grab remote HTML file
	if ( false === ( $html = get_transient( 'rella_remote_html' ) ) ) {
		...
		set_transient( 'rella_remote_html', $data, 24 * HOUR_IN_SECONDS );
 ...
 * @return number
 */
function rella_get_twitter_followers( $screenName = '', $consumerKey= '', $consumerSecret= '' ) {
 ...
 * @return string
 */
function rella_format_num( $num, $precision = 2 ) {
 ...
 * @return string
 */
function rella_get_facebook_likes( $page_link, $facebook_access_token ){

	// Check for transient, if none, grab number of likes
	if ( false === ( $fb_likes = get_transient( 'rella_facebook_likes' ) ) ) {
		...
		set_transient( 'rella_facebook_likes', $data_likes, 24 * HOUR_IN_SECONDS );
 ...
 * @return string
 */
function rella_get_google_plus_followers( $page_id = '', $google_api_key = '' ) {

	// Check for transient, if none, grab number of followers
	if ( false === ( $gplus_followers = get_transient( 'rella_gplus_followers' ) ) ) {
		...
		set_transient( 'rella_gplus_followers', $data_circled, 24 * HOUR_IN_SECONDS );
 ...
 * @return string
 */
function rella_get_behance_followers( $page_link, $behance_api_key = '' ) {

	// Check for transient, if none, grab number of followers
	if ( false === ( $b_followers = get_transient( 'rella_behance_followers' ) ) ) {
		...
		set_transient( 'rella_behance_followers', $data_followers, 24 * HOUR_IN_SECONDS );
 ...
 * @return string
 */
function rella_get_dribbble_followers( $dribbble_access_token ) {

	// Check for transient, if none, grab number of followers
	if ( false === ( $drbbb_followers = get_transient( 'rella_dribbble_followers' ) ) ) {
		...
		set_transient( 'rella_dribbble_followers', $data_followers, 24 * HOUR_IN_SECONDS );
 ...
 * @return string
 */
function rella_get_pinterest_followers( $url ) {

	// Check for transient, if none, grab number of followers
	if ( false === ( $pinterest_followers = get_transient( 'rella_pinterest_followers' ) ) ) {
		...
		set_transient( 'rella_pinterest_followers', $meta_followers, 24 * HOUR_IN_SECONDS );
 ...
 * @return string
 */
function rella_get_instagram_followers( $url ) {

	// Check for transient, if none, grab number of followers
	if ( false === ( $instagram_followers = get_transient( 'rella_instagram_followers' ) ) ) {

		$raw = rella_get_remote_html( $url ); 
		...
		set_transient( 'rella_instagram_followers', $instagram_count, 24 * HOUR_IN_SECONDS );
...
if( ! function_exists( 'rella_portfolio_share' ) ) {
	/**
	...
	function rella_portfolio_share( $post_type = 'post', $args = array() ) {
		...
		<ul class="<?php echo $class ?>">
			...
			<li><a target="_blank" href="https://twitter.com/home?status=<?php echo $url ?> - <?php echo get_the_title(); ?>"><?php echo $hash['tw'][$style] ?></a></li>
			...
			<li><a target="_blank" href="https://www.linkedin.com/shareArticle?mini=true&url=<?php echo $url ?>&amp;title=<?php echo urlencode( get_the_title() ); ?>&amp;source=<?php echo get_bloginfo( 'name' ); ?>"><?php echo $hash['li'][$style] ?></a></li>
		</ul>
		<?php
